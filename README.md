...
The Settings page now has a real Groq API key / Ollama server URL form (`GET`/`POST /api/settings`) — a first-time user can paste their own free-tier Groq key directly in the app instead of hand-editing a `.env` file. It writes only the specific field changed (an env-file upsert that preserves every other line untouched, including keys the page doesn't even show — GitHub PAT, Tavily), and a saved change needs an app restart to take effect, since the LLM router is built once at startup. GitHub's own PAT/Device Flow connection still works the same way it always did (see [Setup](#setup)).
