...
*Live session: the interviewer's actual question ("Why did you choose SFTP over a message queue for ingesting subscriber data in the MVNO intelligence hub?") transcribed word-for-word correctly, with a real, grounded, sourced answer streamed back. Two earlier versions of this screenshot showed the interviewer's audio garbled or hallucinated outright — real bugs, not a feature — see [below](#interviewer-audio-accuracy-and-latency) for the actual root cause and fix, verified live before this screenshot was taken.*
...
Verified live, end-to-end, through the actual built desktop app (not just the API in isolation): after the buffer fix, a real interviewer question spoken through the loopback path — "Why did you choose SFTP over a message queue for ingesting subscriber data in the MVNO intelligence hub?" — was transcribed **word-for-word correctly**, including the acronyms, and the app generated a correctly-grounded answer citing real CV/repo sources. That's the screenshot at the top of this README. Full-answer latency on repeat live-path calls dropped from a measured 6.0s mean/9.5s P90 to ~1.9–2.0s. Not yet re-run through the full 3-pass harness — that's the next thing to do before updating the table itself.
